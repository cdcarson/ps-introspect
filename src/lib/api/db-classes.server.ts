...
> extends AbstractDb {
  ...
  async findMany(input: {
    ...
    const where = getWhere(input.where, this.name);
    const orderBy = getOrderBy(input.orderBy, this.name);
    const limit = getLimitOffset(input.paging);
    const select =
      Array.isArray(input.select) && input.select.length > 0
        ? input.select.map((k) => bt([this.name, k]))
        : raw('*');
    const query = sql`
        SELECT
          ${select}
  ...
  async create(input: { data: CreateData }): Promise<PrimaryKey> {
    ...
    const query = sql`INSERT INTO ${bt(this.name)} (${join(
      names
  ...
  async updateWhere(input: {
    ...
    const updates: Sql[] = keys.map((k) => {
      if (input.data[k] === null) {
        return sql`${bt([this.name, k])} = NULL`;
      } else if (this.jsonKeys.includes(k)) {
        return sql`${bt([this.name, k])} = ${JSON.stringify(input.data[k])}`;
      } else {
        return sql`${bt([this.name, k])} = ${input.data[k]}`;
      }
    });

    const where = getWhere(input.where, this.name);
    const query = sql`UPDATE ${bt(this.name)} SET ${join(
      updates
    )} WHERE ${where}`;
  ...
  async deleteWhere(input: {
    ...
    const where = getWhere(input.where, this.name);
    const query = sql`DELETE FROM ${bt(this.name)} WHERE ${where}`;
    return await this.execute(query);
  }
